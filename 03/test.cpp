...
{
    Matrix m(3, 2);
    if (m.getRows() != 3 || m.getColumns() != 2 || m.mem_allocated != true) {
        return false;
    }
    else
        return true;
...
bool test2() //проверим что конструктор копирования верно создает размер матрицы и аллоцирует память
{
    Matrix m(2, 2);
    Matrix m2(m);
    if (m.getRows() != m2.getRows() || m.getColumns() != m2.getColumns() || m2.mem_allocated != true) {
        return false;
    }
    else
        return true;
...
bool test12() //проверим работу  оператора !=
{
    Matrix m(2, 2);
    m[0][0] = 1;
    m[0][1] = 2;
...
bool test13() //проверим работу  оператора [][] с константными значениями
{
    Matrix m(2, 2);
    m[0][0] = 1;
    m[0][1] = 2;
